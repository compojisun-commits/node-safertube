...
export default function SettingsModal({ onClose }) {
    ...
    const loadSettings = async () => {
      ...
      try {
        ...
        if (userDoc.exists()) {
          ...
          setRole(data.role || 'homeroom');
          setGrade(data.grade || null);
          setTargetGrades(data.target_grades || []);
          setTargetSubjects(data.target_subjects || SUBJECTS.map(s => s.id));
          setFolderHierarchy(data.folder_hierarchy || 'grade-semester-subject');
        }
      } catch (error) {
  ...
  useEffect(() => {
    setHasChanges(true);
  }, [role, grade, targetGrades, targetSubjects, folderHierarchy]);
  ...
  const handleSave = async () => {
    ...
    const confirmResult = await Swal.fire({
      ...
      html: `
        <p style="color: #64748b; font-size: 14px;">
          변경된 설정에 맞춰 새로운 폴더가 추가됩니다.
        </p>
        <p style="color: #94a3b8; font-size: 12px; margin-top: 8px;">
          (기존 폴더와 저장된 영상은 유지됩니다)
        </p>
      `,
      icon: 'question',
    ...
    try {
      ...
      // 2. 폴더 추가 생성 (기존 폴더는 유지, 새로운 폴더만 추가)
      ...
      <div className="onboarding-modal settings-modal">
        ...
        <div className="onboarding-content settings-content">
          ...
          <div className="settings-section">
            <label className="settings-label">역할</label>
          ...
          <div className="settings-section">
            <label className="settings-label">
              {role === 'homeroom' ? '담당 학년' : '담당 학년 (복수 선택)'}
            </label>
          ...
          {/* 폴더 계층 구조 선택 */}
          <div className="settings-section">
            <label className="settings-label">📂 폴더 계층 구조</label>
