// 프론트엔드에서 Firebase Functions를 통해 영상 분석
...
export async function analyzeShortVideo(
  ...
  try {
    ...
    try {
      ...
      // CORS 에러는 정상 - 자막 없이 분석 진행
      console.log("[자막 없음] 영상 직접 분석으로 진행합니다.");
    }
...
${allWarnings
  ...
  .map(
    (w) =>
      `- [${(w.category || "unknown").toUpperCase()}/${w.severity}] ${w.startTime}~${w.endTime}: ${w.description}`
  )
  .join("\n")}
${allWarnings.length > 50 ? `(외 ${allWarnings.length - 50}건 더 있음 - 유사 패턴)` : ""}
...
경고 목록에 '음주', '소주', '술' 관련 내용이 있다면 drug 카테고리를 반드시 caution 이상으로 설정하세요!`,
    ...
    const defaultRatings = {
        level: "safe", score: 100, description: "특별한 유해 요소가 발견되지 않았습니다."
    };
    ...
    return {
      ...
      safetyDescription: summaryResult.safetyDescription || "분석이 완료되었습니다.",
...
export async function analyzeVideoQuick(videoUrl, videoId, gradeLevel, onProgress) {
  try {
    ...
    const analyzeQuick = httpsCallable(functions, 'analyzeVideoQuick');
    ...
    if (!result.data.success) {
      throw new Error(result.data.error || "분석 실패");
    }
